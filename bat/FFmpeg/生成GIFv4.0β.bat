::日志功能--日志开关(0|1)/N日志路径(PATH)/N日志大小(n/B)
@echo off
...
if %sf%==0 (set sc=<nul)
if %bs%==0 (set sp=<nul)
if %cut%==0 (set crop=<nul)
if %sf%==1 (set "sc=scale=%c%x%k%,")
if %bs%==1 (set "sp=setpts=%m%*PTS,")
if %cut%==1 (set "crop=crop=%cv%,")
ffmpeg -hide_banner -ss %s% -t %t% -i %p% -map 0:v -vf "%crop%%sc%%sp%fps=%r%,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" %o%
ffplay %o% -loop 99
if %lsw%==0 (goto PM)
...
echo -ss %s% -t %t% -vf "%crop%%sc%%sp%fps=%r%">>"%lp:"=%\%sd%--%st%.log"
ren %lf% "%sd%--%st%--开始时间%s::=：%--持续时间%t::=：%--缩放%sf%--变速%bs%--裁切%cut%.gif"
:PM
choice /m 是否符合预期
if %errorlevel%==1 (goto Z)
if exist %o% (del /q %o%)
echo -ss %s% -t %t% -vf "%crop%%sc%%sp%fps=%r%"
choice /c abcdefghij /m 重新设值:A开始时间;B持续时间;C帧率;D缩放;E变速;F取消缩放;G取消变速;H更改输出路径名;I取消裁切;J裁切

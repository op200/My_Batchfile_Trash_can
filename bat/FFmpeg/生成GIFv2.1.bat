...
goto Z
:C
ffmpeg -t %t% -ss %s% -i %p% -vf "scale=%c%x%k%,fps=%r%,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" %o%
goto Z
...
goto Z
:E
ffmpeg -t %t% -ss %s% -i %p% -vf "scale=%c%x%k%,fps=%r%,setpts=%m%*PTS,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" %o%

...
set /p cp=输入对照文件夹路径
set /p k=输入二者比值k=
echo %k%|findstr "^[1-9][0-9]*$">nul&&echo.||echo "k=%k%" don't be allowed&&pause&&goto A
choice /m 是否确认?
...
echo.&echo.&echo.
set v=1
for /f "delims=" %%c in (cn.txt) do (for /f "delims=" %%i in (ip.txt) do (echo %%~nxi
echo →→%%~nc-1%%~xi
for /f %%v in ("%v%") do (%cmd%)
goto MB))
:MB
...
if %sk% GEQ %k% (goto MC)
set /a v=sk+1
for /f "delims=" %%c in (cn.txt) do (for /f "skip=%sk% delims=" %%i in (ip.txt) do (echo %%~nxi
echo →→%%~nc-%v%%%~xi
for /f %%v in ("%v%") do (%cmd%)
goto MBC))
:MC
...
for /f "skip=%sk1% delims=" %%c in (cn.txt) do (for /f "skip=%sk2% delims=" %%i in (ip.txt) do (echo %%~nxi
echo →→%%~nc-%v%%%~xi
for /f %%v in ("%v%") do (%cmd%)
goto MCCB))
if %mode%==1 (goto F)
